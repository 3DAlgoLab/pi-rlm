...
function workspaceFromUnknown(value) {
    ...
    if (value && typeof value === "object" && !Array.isArray(value))
        return ensureWorkspaceShape(value);
    return undefined;
}
    ...
    for (let i = branch.length - 1; i >= 0; i--) {
        ...
        if (entry?.type === "message") {
            ...
            if (message?.role !== "toolResult")
                continue;
            if (message.toolName !== "rlm_exec" && message.toolName !== "rlm_reset")
                continue;
...
export function findLatestWorkspaceInBranch(branch) {
    for (let i = branch.length - 1; i >= 0; i--) {
        const entry = branch[i];
        if (entry?.type !== "custom" || entry.customType !== RLM_WORKSPACE_TYPE)
            continue;
        const data = entry.data;
...
export function findBootstrapSnapshot(ctx) {
    ...
    for (let i = branch.length - 1; i >= 0; i--) {
        const entry = branch[i];
        if (entry.type !== "custom" || entry.customType !== "rlm-child-bootstrap")
            continue;
        const data = entry.data;
        if (!data?.state)
            return undefined;
