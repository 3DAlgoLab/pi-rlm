...
const WORKSPACE_RUNTIME_PATH = "globalThis.workspace";
const PARENT_STATE_RUNTIME_PATH = "globalThis.parentState";
const INPUT_RUNTIME_PATH = "globalThis.input";
...
function normalizeArtifactList(value) {
	...
	return value.filter((item) => isRecord(item) && typeof item.childId === "string");
}
...
function extractReferenceList(value) {
	if (!Array.isArray(value)) return undefined;
	const refs = value.flatMap((item) => {
		if (!item || typeof item !== "object" || Array.isArray(item)) return [];
		const record = item;
		return [record.ref, record.path, record.id].filter(
...
function buildDerivedActiveContext(workspace) {
	const goal =
	...
	const updatedAt =
		typeof workspace.meta?.updatedAt === "string" ? workspace.meta.updatedAt : workspace.lastChildArtifact?.producedAt;
	return {
		...(goal ? { goal } : {}),
...
function refreshWorkspaceProjection(workspace) {
	const derived = buildDerivedActiveContext(workspace);
	const existing = isRecord(workspace.activeContext) ? workspace.activeContext : {};
	workspace.activeContext = {
		...
		...(existing.currentFindingsRefs && !derived.currentFindingsRefs
			? { currentFindingsRefs: existing.currentFindingsRefs }
			: {}),
		...(derived.currentArtifactRefs ? { currentArtifactRefs: derived.currentArtifactRefs } : {}),
		...(existing.currentArtifactRefs && !derived.currentArtifactRefs
			? { currentArtifactRefs: existing.currentArtifactRefs }
			: {}),
	...
	if (workspace.activeContext.currentArtifactRefs?.length)
		meta.activeArtifactRefs = [...workspace.activeContext.currentArtifactRefs];
	else delete meta.activeArtifactRefs;
	workspace.meta = meta;
	return workspace;
}
function ensureArtifactIndexShape(value) {
...
export function ensureWorkspaceShape(value) {
	...
	workspace.childArtifactSummaries = Array.isArray(workspace.childArtifactSummaries)
		? workspace.childArtifactSummaries.filter((item) => isRecord(item) && typeof item.id === "string")
	...
	workspace.meta = isRecord(workspace.meta) ? { version: 1, ...workspace.meta } : { version: 1 };
	return refreshWorkspaceProjection(workspace);
}
function extractArtifactFiles(artifact) {
	const direct = normalizeStringList(artifact.files);
	if (direct) return direct;
	if (artifact.data && isRecord(artifact.data)) {
		const nestedFiles = normalizeStringList(artifact.data.files);
		if (nestedFiles) return nestedFiles;
	}
	return undefined;
}
function extractArtifactTags(artifact) {
	const direct = normalizeStringList(artifact.tags);
	if (direct) return direct;
	if (artifact.data && isRecord(artifact.data)) {
		const nestedTags = normalizeStringList(artifact.data.tags);
...
function rebuildArtifactIndex(artifacts) {
	const byId = {};
	const byTag = {};
	const byFile = {};
	const recentIds = [];
...
export function recordArtifact(workspaceValue, artifactValue, maxArtifacts = MAX_WORKSPACE_ARTIFACTS) {
...
function createScalarPreview(value) {
	if (value === null || typeof value === "number" || typeof value === "boolean") return value;
	if (typeof value === "string") return value.length <= 120 ? value : `${value.slice(0, 117)}...`;
	return undefined;
}
export function buildValueManifest(value, path, options = {}) {
	...
	if (typeof value === "string")
		return { path, type: "string", length: value.length, preview: createScalarPreview(value) };
...
export function buildStateManifest(state) {
...
export function buildWorkspaceManifest(workspaceValue, options = {}) {
	if (!isRecord(workspaceValue)) return undefined;
	const workspace = ensureWorkspaceShape(workspaceValue);
	const rawKeys = options.sectionKeys?.length
		? options.sectionKeys
		: Object.keys(workspace).filter(
				(key) => key !== "artifactIndex" && key !== "lastChildArtifact" && key !== "childArtifactSummaries",
			);
	const sectionKeys = rawKeys.slice(0, options.sectionLimit ?? DEFAULT_MANIFEST_SECTION_LIMIT);
	const sections = {};
	for (const key of sectionKeys) {
		if (!(key in workspace)) continue;
		sections[key] = buildValueManifest(workspace[key], `${WORKSPACE_RUNTIME_PATH}.${key}`);
	}
	return {
		version: 1,
		runtime: {
		...
		...(workspace.artifactIndex?.recentIds?.length
			? { recentArtifactIds: workspace.artifactIndex.recentIds.slice(-DEFAULT_ARTIFACT_PREVIEW_LIMIT) }
			: {}),
		...(options.relevantArtifacts?.length ? { relevantArtifacts: options.relevantArtifacts } : {}),
	};
}
export function buildWorkspaceWorkingSetSummary(workspaceValue) {
	if (!isRecord(workspaceValue)) return undefined;
	const workspace = ensureWorkspaceShape(workspaceValue);
...
function buildRetentionConsolidationRef(summary) {
	return {
		kind: "workspace-path",
...
function refreshRetentionLeases(leases, latestTurnIndex, now) {
	return (leases ?? [])
		.map((lease) => {
...
function consolidateLeases(leases, latestTurnIndex, consolidatedTo, expiresAfterTurns, now) {
...
function fingerprintRetentionMetrics(metrics) {
	return [
		metrics.keptMessages,
		metrics.prunedMessages,
		metrics.placeholderMessages,
		metrics.retainedTurns,
		metrics.prunedTurns,
		metrics.activeContextSummary ?? "",
	].join(":");
}
export function recordRetentionLease(workspaceValue, input) {
	if (!isRecord(workspaceValue)) return undefined;
...
export function recordRetentionMetrics(workspaceValue, latestMetrics, latestTurnIndex, policy) {
	if (!isRecord(workspaceValue)) return undefined;
	const workspace = ensureWorkspaceShape(structuredClone(workspaceValue));
	const now = new Date().toISOString();
	...
	const summary =
		latestMetrics.activeContextSummary ??
		(keepLatestSurfaceSummary ? currentRetention.latestSurfaceSummary : undefined);
	...
	const currentFingerprint = currentRetention.latestMetrics
		? fingerprintRetentionMetrics(currentRetention.latestMetrics)
		: undefined;
	...
	if (currentRetention.latestTurnIndex === latestTurnIndex && currentFingerprint === nextFingerprint && existingLease)
		return workspace;
	const consolidatedLeases = consolidateLeases(
		currentRetention.leases,
		latestTurnIndex,
		consolidationRefs,
		expiresAfterTurns,
		now,
	);
	const refreshedLeases = refreshRetentionLeases(consolidatedLeases, latestTurnIndex, now).filter(
...
export function buildWorkspacePointerHints(workspaceValue) {
	if (!isRecord(workspaceValue)) return undefined;
...
function artifactScore(summary, tokens, role) {
	let score = 0;
	if (summary.role === role) score += 2;
	const haystacks = [summary.summary ?? "", ...(summary.files ?? []), ...(summary.tags ?? [])].map((value) =>
		value.toLowerCase(),
	);
...
export function selectRelevantArtifacts(workspaceValue, options = { prompt: "", role: "general" }) {
...
export function selectRelevantWorkspaceSectionKeys(role, workspaceValue) {
	if (!isRecord(workspaceValue)) return [];
	const workspace = ensureWorkspaceShape(workspaceValue);
	const byRole = {
...
export function attachInternalLlmQueryContext(input, context) {
...
export function splitInternalLlmQueryContext(input) {
	...
	const workspace = isRecord(internal) || internal === null ? (internal?.workspace ?? undefined) : undefined;
