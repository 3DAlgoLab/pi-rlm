...
function extractReferenceList(value) {
    ...
    const refs = value.flatMap((item) => {
        if (!item || typeof item !== "object" || Array.isArray(item))
            return [];
        const record = item;
        return [record.ref, record.path, record.id].filter((candidate) => typeof candidate === "string" && candidate.trim().length > 0);
    });
...
function buildDerivedActiveContext(workspace) {
    ...
    const currentFindingsRefs = Array.from(new Set([
    ...
    const summaryParts = [];
    if (goal)
        summaryParts.push(`Goal: ${goal}`);
    if (currentPlan?.length)
        summaryParts.push(`Plan: ${currentPlan.slice(0, 3).join(" · ")}`);
    if (relevantFiles?.length)
        summaryParts.push(`Files: ${relevantFiles.slice(0, 4).join(", ")}`);
    if (currentQuestions?.length)
        summaryParts.push(`Questions: ${currentQuestions.slice(0, 3).join(" · ")}`);
    if (currentArtifactRefs?.length)
        summaryParts.push(`Artifacts: ${currentArtifactRefs.slice(-4).join(", ")}`);
...
function refreshWorkspaceProjection(workspace) {
    ...
    if (workspace.activeContext.currentPlan)
        meta.activePlanRef = "globalThis.workspace.activeContext.currentPlan";
    else
        delete meta.activePlanRef;
    if (workspace.activeContext.currentArtifactRefs?.length)
        meta.activeArtifactRefs = [...workspace.activeContext.currentArtifactRefs];
    else
        delete meta.activeArtifactRefs;
    workspace.meta = meta;
    return workspace;
}
function ensureArtifactIndexShape(value) {
    const index = isRecord(value) ? value : {};
    if (!isRecord(index.byId))
        index.byId = {};
    if (!isRecord(index.byTag))
        index.byTag = {};
    if (!isRecord(index.byFile))
        index.byFile = {};
    if (!Array.isArray(index.recentIds))
        index.recentIds = [];
    return index;
}
...
export function buildValueManifest(value, path, options = {}) {
    ...
    if (value === null)
        return { path, type: "null", preview: null };
    if (Array.isArray(value)) {
        const preview = value
...
export function buildStateManifest(state) {
    if (!state || !isRecord(state) || Object.keys(state).length === 0)
        return undefined;
...
function refreshRetentionLeases(leases, latestTurnIndex, now) {
    return (leases ?? [])
        .map((lease) => {
        if (lease.status !== "consolidated" || typeof lease.expiresAfterTurns !== "number")
            return lease;
        if (latestTurnIndex - lease.turnIndex < lease.expiresAfterTurns)
            return lease;
        return {
            ...lease,
            status: "expired",
...
function consolidateLeases(leases, latestTurnIndex, consolidatedTo, expiresAfterTurns, now) {
    return (leases ?? []).map((lease) => {
        if (lease.status !== "live")
            return lease;
        if (lease.turnIndex > latestTurnIndex)
            return lease;
        return {
            ...lease,
            status: "consolidated",
...
export function splitInternalLlmQueryContext(input) {
    if (!isRecord(input) || !(INTERNAL_LLM_QUERY_CONTEXT_KEY in input))
