...
function buildResumeSnapshot(artifact, state, workspace) {
	const base = artifact?.snapshot ? structuredClone(artifact.snapshot) : { version: 1, bindings: {}, entries: [] };
...
export function buildForcedFinalizePrompt(args) {
	...
	sections.push(
		"- Inspect globalThis.workspace.activeContext first and use pointers instead of replaying transcript history.",
	);
	sections.push(
		"- If reusable findings already exist in runtime/workspace, preserve that structure and finalize from it.",
	);
...
function buildChildArtifact(sessionManager, options) {
	...
	const artifact = {
		...
		...(workspace === undefined
			? {}
			: { workspace: workspace === null ? null : ensureWorkspaceShape(structuredClone(workspace)) }),
	};
	return artifact;
}
...
export async function runChildQuery(input, ctx, options) {
	...
	const parentWorkspace =
		inputWorkspace === undefined ? undefined : inputWorkspace === null ? null : ensureWorkspaceShape(inputWorkspace);
	...
	try {
		...
		const builtInToolNames = resolveBuiltInToolSet(
			resolveBuiltInToolNames(normalized.tools, options.parentActiveTools),
		);
