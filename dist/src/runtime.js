    ...
    function workerMain() {
            ...
            try {
                ...
                const text = JSON.stringify(value, (_key, inner) => {
                    ...
                    if (inner instanceof Map)
                        return { __type: "Map", entries: Array.from(inner.entries()).slice(0, 5) };
                    if (inner instanceof Set)
                        return { __type: "Set", values: Array.from(inner.values()).slice(0, 5) };
        ...
        const inspect = () => {
            ...
            const table = entries.length
                ? entries
                    .map((entry) => `- ${entry.name}: ${entry.type}${entry.size ? ` (${entry.size})` : ""}${entry.preview ? ` = ${entry.preview}` : ""}${entry.restorable ? "" : " [preview-only]"}`)
                    .join("\n")
                : "(runtime empty)";
        ...
        const callParent = (method, payload) => new Promise((resolve, reject) => {
        ...
        const ensureWorkspace = (sandbox) => {
            ...
            if (!Array.isArray(workspace.childArtifacts))
                workspace.childArtifacts = [];
            if (!Array.isArray(workspace.childArtifactSummaries))
            ...
            if (!Array.isArray(workspace.artifactIndex.recentIds))
        ...
        const normalizeStringList = (value) => Array.isArray(value) ? value.filter((item) => typeof item === "string" && item.trim().length > 0) : undefined;
        ...
        const buildWorkspaceActiveContext = (workspace) => {
            ...
            const artifactIndex = workspace.artifactIndex && typeof workspace.artifactIndex === "object" && !Array.isArray(workspace.artifactIndex)
            ...
            const recentArtifactIds = Array.isArray(artifactIndex?.recentIds)
                ...
                : [];
            const currentArtifactRefs = Array.from(new Set([
                ...(Array.isArray(workspace.childArtifacts)
                    ? workspace.childArtifacts.flatMap((artifact) => {
                        if (!artifact || typeof artifact !== "object" || Array.isArray(artifact))
                            return [];
                        const record = artifact;
                        return [record.id, record.childId].filter((candidate) => typeof candidate === "string" && candidate.trim().length > 0);
                    })
                    : []),
                ...recentArtifactIds,
            ])).slice(-6);
            ...
            const meta = workspace.meta && typeof workspace.meta === "object" && !Array.isArray(workspace.meta) ? workspace.meta : undefined;
        ...
        const mergeChildArtifactIntoWorkspace = (sandbox, artifact) => {
            if (!artifact || typeof artifact !== "object" || !canClone(artifact))
                return;
        ...
        const createSandbox = (consoleProxy, finalState) => {
            ...
            sandbox.llmQuery = async (input) => {
                ...
                if (childArtifact)
                    mergeChildArtifactIntoWorkspace(sandbox, childArtifact);
                return publicValue;
            };
            sandbox.final = (value) => {
...
export class RuntimeSession {
    ...
    onMessage(message) {
        ...
        if (message?.type === "rpc" && message.method === "llmQuery") {
            void (async () => {
                try {
                    if (!this.currentLlmQuery)
                        throw new Error("llmQuery is not available in this execution context");
