...
import { buildRlmModeAppendix, DEFAULT_RLM_PROMPT_MODE, findRlmPromptMode, getRlmPromptModeLabel, RLM_PROMPT_MODE_TYPE, } from "./prompt-mode.js";
...
function findRlmModeEnabled(ctx) {
    ...
    for (let i = branch.length - 1; i >= 0; i--) {
        ...
        if (entry.type !== "custom" || entry.customType !== RLM_MODE_TYPE)
            continue;
...
function applyModeWidget(ctx, enabled, promptMode, root) {
    if (!root || !ctx.hasUI)
        return;
    ctx.ui.setWidget("rlm-mode", enabled ? [`${PINK}RLM MODE${RESET} ${getRlmPromptModeLabel(promptMode)} recursive runtime active`] : undefined, { placement: "aboveEditor" });
}
function applyStatus(ctx, enabled, promptMode, root, stats) {
    if (!root || !ctx.hasUI)
        return;
    if (!enabled) {
    ...
    const details = theme.fg("dim", ` · d ${nextStats.depth}/${nextStats.maxDepth} · exec ${nextStats.execCount} · child ${nextStats.childTurns > 0 ? `${nextStats.childQueryCount}/${nextStats.childTurns}t` : `${nextStats.childQueryCount}`} · vars ${nextStats.runtimeVarCount} · act ${nextStats.activeContextRefCount} · leaf ${nextStats.leafToolCount}`);
    ctx.ui.setStatus("rlm-stats", header + details);
}
async function restoreRuntime(manager, initializedKeys, ctx) {
    const key = getSessionRuntimeKey(ctx);
    const runtime = manager.getOrCreate(key);
    const workspace = findLatestWorkspace(ctx);
    const restoredSnapshot = composeRuntimeSnapshot(findLatestSnapshot(ctx) ?? findBootstrapSnapshot(ctx), workspace && workspace !== null ? markRunningChildrenInterrupted(workspace) : workspace);
    await runtime.restore(restoredSnapshot);
    initializedKeys.add(key);
    return runtime;
}
async function getRuntime(manager, initializedKeys, ctx) {
    const key = getSessionRuntimeKey(ctx);
    if (!initializedKeys.has(key))
        return restoreRuntime(manager, initializedKeys, ctx);
    return manager.getOrCreate(key);
}
function computeStats(ctx, options, snapshot) {
...
function markRunningChildrenInterrupted(workspace) {
...
function applyChildProgress(children, event) {
    switch (event.type) {
        case "start":
        ...
        case "tool_end": {
            ...
            if (!child)
                return;
            if (child.activeTool === event.toolName)
...
function renderChildLine(child, theme, expanded) {
    ...
    if (child.activeTool)
        text += theme.fg("muted", ` · ${child.activeTool}`);
    if (expanded && child.summary)
        text += `\n  ${theme.fg("muted", child.summary)}`;
    if (expanded && child.error)
        text += `\n  ${theme.fg("error", child.error)}`;
    return text;
}
function persistWorkspaceEntry(pi, currentWorkspace, previousWorkspace) {
    const nextWorkspace = currentWorkspace ?? null;
    const lastWorkspace = previousWorkspace ?? null;
    if (isDeepStrictEqual(nextWorkspace, lastWorkspace))
        return;
    pi.appendEntry(RLM_WORKSPACE_TYPE, { workspace: nextWorkspace });
}
function renderRlmExecResult(result, options, theme) {
...
export function createRlmExtensionFactory(options) {
    return function installRlm(pi) {
        ...
        const captureRetention = (messages, ctx) => {
            if (!isRlmActive()) {
        ...
        const emitRetentionEntry = (turnIndex) => {
            if (!pendingRetentionEntry ||
        ...
        const getWorkspaceBase = (ctx) => pendingWorkspace ?? findLatestWorkspace(ctx);
        const queueWorkspaceUpdate = (next) => {
            if (!next)
                return;
            pendingWorkspace = next;
        };
        const flushWorkspaceUpdates = (ctx) => {
            if (!pendingWorkspace)
                return;
        ...
        const syncRetentionWorkspace = (ctx, retention) => {
            if (!retention)
                return;
        ...
        pi.registerTool({
            name: "rlm_exec",
            label: "RLM Exec",
            description: "Execute JavaScript in a persistent runtime with live variables. Persist state by assigning to globalThis.<name>.",
            promptSnippet: "Use this as the persistent coordinator workspace for multi-file or multi-step tasks. Keep durable state in globalThis.workspace and globalThis.workspace.activeContext. Helpers: final(), inspectGlobals(), llmQuery({ prompt, ... }).",
            promptGuidelines: [
            ...
            parameters: Type.Object({
                code: Type.String({
                    description: "JavaScript to execute. Helpers available inside the runtime: inspectGlobals(), final(value), llmQuery({ prompt, role, state, tools, budget, output }).",
                }),
            }),
            ...
            renderResult(result, { expanded, isPartial }, theme) {
                return renderRlmExecResult(result, { expanded, isPartial }, theme);
            },
            async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
                ...
                const emitLiveUpdate = () => {
                    const now = Date.now();
                    if (now - lastUpdateAt < LIVE_UPDATE_THROTTLE_MS)
                        return;
                    lastUpdateAt = now;
                    _onUpdate?.({
        ...
        if (options.root) {
            const setPromptMode = async (mode, ctx) => {
                ...
                handler: async (args, ctx) => {
                    ...
                    switch (action.type) {
                        ...
                        case "inspect": {
                            ...
                            applyStatus(ctx, rlmModeEnabled, rlmPromptMode, true, stats);
                            if (ctx.hasUI)
                                await ctx.ui.editor("RLM runtime", inspection.table);
                            else
                                pi.sendMessage({ customType: "rlm-inspect", content: inspection.table, display: true });
                            return;
                        }
                        case "reset": {
                            const runtime = await getRuntime(manager, initializedKeys, ctx);
