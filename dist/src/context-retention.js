...
function buildToolResultPlaceholder(message, lease) {
	...
	return {
		...
		content: [
			{
				type: "text",
				text: `${toolName} output consolidated into ${ref}. Inspect runtime/workspace instead of replaying prior tool output.`,
			},
		],
	};
}
function maybeThinToolResultMessage(message, workspace, currentTurnIndex) {
	...
	if (message.isError === true) return message;
...
export function applyRetentionPolicy(messages, options = {}) {
	...
	const summaryPrelude = policy.keepLatestSurfaceSummary
	...
	if (prunedTurnIndexes.size > 0 && policy.replaceExpiredWithReference) {
		const placeholderTimestamp =
			retainedTurnMessages[0]?.timestamp ?? retainedPrelude[retainedPrelude.length - 1]?.timestamp ?? Date.now();
